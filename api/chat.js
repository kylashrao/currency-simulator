...
export default async function handler(req, res) {
    ...
    if (!apiKey) {
        return res.status(200).json({ text: 'Error: GEMINI_API_KEY environment variable is missing on Vercel.' });
    }
...
- Use clear markdown formatting (bolding, bullet points) for readability.`;
    ...
    try {
        ...
        const errMsg = err.name === 'AbortError' ? 'Request timed out connecting to Google API.' : err.message;
        return res.status(200).json({ text: "Backend Error: " + errMsg });
    }
}